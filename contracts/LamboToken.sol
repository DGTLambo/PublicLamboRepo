...
//                  ------------------ Contract Start Functions ---------------------                //
    ...
    {
        bonusReleaseTime = 1; //TODO: Set this to 48 hours worth of blocks once done testing
...
//                  ------------------ Nitro Implementation ---------------------                //
        ...
        whenNotPausedOrInitialDistribution()
    {
        ...
        if(!_isWhitelistedSender(sender) || !_isWhitelistedSender(recipient)){
            ...
            }else if(isExchangeAddress(recipient)) { //TODO: What if we also require that msg.sender is not the uniswap router? Would this skip liquidity provisions then?
                _updateShortTwap();
                _updateLongTwap();
...
//                  ------------------ User Functions ---------------------                //
    ...
    function calculateCurrentNitroRate(bool isBuy) public view returns (uint256) {
        ...
        // uint256 currentRealTimePrice = getLastShortTwap(); //close to instantaneous but secure TODO Ask Rubik about this
        uint256 currentRealTimePrice = 200;
        
        // uint256 currentTwap = getLastLongTwap();
        uint256 currentTwap = 400;
        uint256 nitro;
